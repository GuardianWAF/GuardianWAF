...
func TestGenerator_Generate(t *testing.T) {
	...
	if patch.Enabled {
		t.Error("auto-generated patch must start disabled until reviewed")
	}
	if patch.ReviewStatus != "pending_review" {
		t.Errorf("review_status = %q, want pending_review", patch.ReviewStatus)
	}
	if patch.Provenance != "nvd:auto-generated" {
		t.Errorf("provenance = %q, want nvd:auto-generated", patch.Provenance)
	}
...
func TestLayer_AutoGeneratedPatchRequiresExplicitApply(t *testing.T) {
	layer := NewLayer(&Config{
		Enabled:       true,
		BlockSeverity: []string{"CRITICAL"},
	})
	patch := &VirtualPatch{
		ID:            "VP-CVE-REVIEW",
		CVEID:         "CVE-2026-0001",
		Name:          "Generated review gate",
		Severity:      "CRITICAL",
		Action:        "block",
		Score:         90,
		Patterns:      []PatchPattern{{Type: "path", Pattern: "/generated", MatchType: "exact"}},
		AutoGenerated: true,
	}
	layer.AddPatch(patch)

	stored := layer.GetPatch("VP-CVE-REVIEW")
	if stored == nil {
		t.Fatal("expected stored patch")
	}
	if stored.Enabled {
		t.Fatal("auto-generated patch was enabled before review")
	}
	if stored.ReviewStatus != "pending_review" {
		t.Fatalf("review_status = %q, want pending_review", stored.ReviewStatus)
	}

	result := layer.Process(&engine.RequestContext{Method: "GET", Path: "/generated"})
	if result.Action == engine.ActionBlock {
		t.Fatal("pending-review generated patch blocked traffic before explicit apply")
	}

	if !layer.EnablePatchBy("VP-CVE-REVIEW", "security-reviewer") {
		t.Fatal("EnablePatchBy returned false")
	}
	applied := layer.GetPatch("VP-CVE-REVIEW")
	if applied == nil || !applied.Enabled {
		t.Fatal("expected applied patch to be enabled")
	}
	if applied.ReviewStatus != "applied" {
		t.Fatalf("review_status = %q, want applied", applied.ReviewStatus)
	}
	if applied.ReviewedBy != "security-reviewer" || applied.AppliedBy != "security-reviewer" {
		t.Fatalf("review/apply actors not recorded: reviewed_by=%q applied_by=%q", applied.ReviewedBy, applied.AppliedBy)
	}
	if applied.ReviewedAt == nil || applied.AppliedAt == nil {
		t.Fatal("review/apply timestamps not recorded")
	}

	result = layer.Process(&engine.RequestContext{Method: "GET", Path: "/generated"})
	if result.Action != engine.ActionBlock {
		t.Fatalf("expected applied generated patch to block, got %v", result.Action)
	}
}
