...
func (d *Dashboard) registerStats(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/v1/ssl/certificates", d.authWrap(d.handleUploadCertificateCompat))
	mux.HandleFunc("DELETE /api/v1/ssl/certificates/{name}", d.authWrap(d.handleDeleteCertificateCompat))
